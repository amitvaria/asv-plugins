...
function asv_tracker_image($step=''){
    ...
    switch($step){
        case 'image_save':    $action .= 'The user attempted to create or update image #'.$id;  $write=1;
                            break;
        case 'image_delete': $action .= 'The user attempted to delete image #'.$id; $write=1;
                            break;
    }
    
    ($write == 1) ? safe_insert(safe_pfx_j('asv_tracker'), 'user="'.$txp_user.'", access=now(), action="'.$action.'"') : '1';
}
